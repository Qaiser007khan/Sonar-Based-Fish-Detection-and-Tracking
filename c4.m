...
videoFile = '2023-07-01_1000.mp4'; % Specify your video file path
vidReader = VideoReader(videoFile);
bgSubtractor = vision.ForegroundDetector('NumGaussians', 3, 'NumTrainingFrames', 150, 'MinimumBackgroundRatio', 0.80);

MaxFrame = 1000;
% Prepare to save data to an Excel file
outputFile = 'fish_2023-07-01_1000.xlsx';
...
scaleFactor = 1.8; % Conversion factor from pixels to cm

maxLostFrames = 15; % Max frames a fish can be undetected before being removed
...
while hasFrame(vidReader) && frameCount < MaxFrame
    ...
    for m = length(trackedFish):-1:1
        if frameCount - trackedFish(m).lastSeen > maxLostFrames
            ...
            fishData{end, 4} = trackedFish(m).firstFrame;    % Frame number (first frame)
...
disp('Fish tracking complete. Data saved to Excel and video saved.');